...
class _GameScreenState extends State<GameScreen> {
  ...
  void _initGame() {
    ...
    _state = GameState(
      ...
      remainingSeconds: _isPractice ? 9999 : _totalSeconds,
    );
  ...
  void _useHint() {
    ...
    if (_state.availablePieces.isEmpty) return;

    // 배치 안 된 조각 중 첫 번째를 강조
    setState(() {
      _hintsUsed++;
      _hintPieceId = _state.availablePieces.first.instanceId;
    });

    // 3초 후 힌트 해제
    Future.delayed(const Duration(seconds: 3), () {
      if (mounted) setState(() => _hintPieceId = null);
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          '\uBE14\uB85D\uD53C\uD2B8  ${_currentIndex + 1}/${widget.puzzles.length}${_isPractice ? "  \uC5F0\uC2B5" : ""}',
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        _ActionBtn(
                          ...
                          label: '\uD78C\uD2B8 ${_maxHints - _hintsUsed}',
                          enabled: _hintsUsed < _maxHints && _state.availablePieces.isNotEmpty,
                          onTap: _useHint,
                          color: Colors.amber,
                        ),
                        const SizedBox(width: 12),
                    ...
                    GridView.builder(
                      ...
                      itemBuilder: (context, i) {
                        ...
                        final isHinted = piece.instanceId == _hintPieceId;
...
with open('lib/screens/game_screen.dart', 'r', encoding='utf-8') as f:
    c = f.read()
lines = c.splitlines()
print(f"Lines: {len(lines)}, Braces: {c.count(chr(123))}/{c.count(chr(125))}")
